...
| 03 | `03-data-model.md` | 业务表结构、关联、数据库约束及持久化规则 | 合法关系能否表达，数据变更后的状态是否一致 |
...
| 07 | `07-api.md` | 业务操作编排、页面使用的内部 API、认证与 Settings 接口、状态校验及错误表达 | Sync、级联预览和确认等操作是否完整，接口是否足以支撑页面 |
...
| B. 业务数据与约束 | 03 | Subscription、Node、Inbound、Outbound、Node Pool、Route 的表结构与约束；DIRECT 的 Route 表达；Node 身份与删除后的持久化规则 | 可以表达全部合法关系；关联数据的最终状态明确 |
