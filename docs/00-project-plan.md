...
当前工作区已有 00～05 编号文档；02、03 已确认，04、05 已完成初稿并待联合评审。06～11 是后续计划产物。文档的审核状态以用户确认记录为准，不能仅凭文件是否存在判断。
...
04、05 各自形成一份可分章节审核的完整设计稿，允许联合核对输入契约与配置映射，分别记录审核结论。联合评审不表示两份文档同时通过；对应实现仍须等待其依赖的设计全部确认。需要调整篇幅或边界时，同步更新计划中的目录、审核包及实现依赖。
