...
| 04 | `04-singbox-design.md` | Node 协议、分享 URI 与订阅内容解析、配置映射、selector 控制、配置检查和二进制管理 | 输入格式、sing-box 实际能力、配置样例和切换行为是否满足需求 |
...
| 07 | `07-api.md` | 业务操作编排、页面使用的内部 API、认证与 Settings 接口、状态校验及错误表达 | Sync、Node 删除级联、预览和确认等操作是否完整，接口是否足以支撑页面 |
...
当前工作区已有 00～03 编号文档；04～09 是后续计划产物。文档的审核状态以用户确认记录为准，不能仅凭文件是否存在判断。
...
起草 04 时，按 Node/Subscription 输入与 sing-box 集成的实际篇幅决定是否拆成独立文档；业务操作编排与 API 契约也可按职责拆分。拆分时同步更新目录、审核包及实现依赖。

02～07 按各自主题记录必要的设计结论；涉及流程、配置映射、状态转换和交互时提供可检查样例，存在未决问题时列出。03 以表结构、关联和约束为主要审核依据。用户确认的设计结论允许对应实现批次开始；未确认的部分继续修订。08、09 可由 AI 先整理测试与部署材料，再由用户审核覆盖和可操作性。`docs/history/` 与 `docs/work-in-progress/` 用于过程记录，不作为已冻结需求或已审核设计的替代依据。
...
| C. Node 与 Subscription 输入 | 04 | 五种 Node 协议、原生插件、单条分享 URI、实际订阅格式、解析与过滤顺序、Refresh 元信息 | 有真实或脱敏的有效/无效样例；不支持输入的处理清楚 |
...
| F. Settings、认证、页面与 API | 05～07 | 各设置的保存和生效时机、认证与会话、Sync/删除的级联预览和事务编排、操作状态校验、桌面/移动页面、内部 API、错误提示和日志展示 | 页面与 API 不改变业务规则；用户能区分管理状态、进程状态与节点角色 |
...
需求基线已经冻结，02 已确认，03 待审核。下一步完成 03 审核，再起草 04；后续文档的具体审核内容见第 5 节。已由用户确认的结论直接作为对应实现依据；尚未确认的，由 AI 整理必要方案和验证材料后提交审核。
