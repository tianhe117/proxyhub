...
本文规定 ProxyHub V1.0 的产品功能与行为，是后续数据模型、状态机、sing-box 集成、页面、API 和验收设计的需求基准。
...
**REQ-GEN-004** 系统服务个人家庭实际使用，不以公共服务、多用户平台、企业部署或通用代理管理平台为目标。

**REQ-GEN-005** 系统的主要行为应简单、确定、可排错，不为低概率异常建立复杂恢复状态、分布式任务、历史任务或企业级高可靠机制。

**REQ-GEN-006** 系统面向单一可信用户，仅实现当前需求必需的基本防护，不为异常场景和未来需求增加额外复杂设计。
...
系统支持 Docker Compose 和 Ubuntu Python/venv 两种部署方式，并保持配置格式一致。

**REQ-DEPLOY-002**

系统支持 Ubuntu 20.04 及以上版本以及 amd64 架构 CPU。不要求支持 Windows、macOS、其他 Linux 发行版、32 位 x86 或 arm64 架构。
...
所有运行控制锁的使用与串行化规则统一遵循 REQ-RUNTIME-007。系统不建立跨进程锁或分布式事务。
...
配置更新不承诺无中断，允许人工 Stop、修改、Start 过程中出现短暂停顿。

---

## 15. 不支持的范围

以下内容不属于 ProxyHub V1.0 的需求：
...
- 旧版（v1/v2）数据库、Settings、运行状态或内部 API 兼容迁移；
