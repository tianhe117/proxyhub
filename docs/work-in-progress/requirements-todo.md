# TODO：第 1～8 章修改后的后续需求同步

第 1～8 章已完成业务模型、sing-box 配置管理及运行控制职责重构。后续章节应以前 8 章为基线同步检查和调整，避免重复定义已有业务规则，并确保健康检测、AUTO、Settings、API 和页面行为保持一致。

## 1. Node 健康检测

整理第 9 章时检查：

- 明确单个 Node 的 TCP、URL 检测流程及最终健康状态判定；
- 明确 Node 健康状态包含的信息、初始化和更新规则；
- 明确 AUTO 检测和人工检测是否使用相同的单 Node 检测流程；
- 明确批量检测可以并发执行，并发数量由配置控制；
- 明确 AUTO 检测、人工检测以及不同人工检测请求之间的并发关系；
- 明确同一 Node 存在并发检测时健康状态的更新规则；
- 明确人工检测是否影响 AUTO Current Node、失败计数及其他 AUTO Runtime State。

Node 健康状态属于 Runtime State，新运行周期开始时由第 8 章统一初始化。第 9 章只定义检测及检测结果在运行周期中的变化规则，不重复定义运行周期。
...
Candidate Node、Fallback Node、Default Node、Current Node、Node Pool priority 和 Runtime State 的基础定义沿用前 8 章，不重复定义。
...
- Settings 修改是否影响 Current Node、Default Node 或已有 AUTO Runtime State；
- 需要重新启动 ProxyHub 或其他组件才能生效的 Settings，其保存和生效规则；
- 检测并发配置与第 9 章的批量检测规则保持一致；
- 控制周期相关配置与第 8 章的后台控制循环保持一致。
