...
| `utils/tcp_check`、`CheckResult`（schemas） | 纯 Python，与引擎无关 |
| `utils/common.py`、`validators.py`、`port.py`（service 段）、`logger.py` | 叶子工具，无引擎耦合 |
...
| 资产 | 改动 |
|------|------|
| `settings.py` | 删 `BIN_REGISTRY`（三引擎）、`BIN_REPOS`（含 obfs-local）、`PROTOCOL_BIN_MAP` 全指向 sing-box（或简化为「协议列表」）；保留路径 helper + settings 持久化 |
