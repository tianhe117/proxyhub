...
```text
Subscription ──< Node >── Outbound Node Pool >── MANUAL / AUTO
                      │                                  │
                      └──── 可被多个 Pool 复用             └──< Route >── Inbound

DIRECT ────────────────────────────────────────────────< Route
```

Subscription 产生订阅 Node；自建 Node 的 `subscription_id` 为 `NULL`。Node 是全局实体，Pool 仅记录它与 MANUAL/AUTO 的成员关系及 priority。一个 MANUAL/AUTO 可以被多条 Route 引用，这些 Route 共用同一个 Pool 和 Default Node。DIRECT 是系统对象，稳定标识为 `direct`，没有数据库行。

Subscription、Node、Inbound、MANUAL/AUTO 和 Route 使用正整数 `id`，新建时由 SQLite 分配；Node Pool 成员以 `outbound_id + node_id` 标识。已存在的订阅 Node 在同名 Sync 更新时保持原 `id`。`direct` 与整数 Outbound ID 分属不同标识空间；Route 通过目标类型和可空 Outbound ID 表达二者。

## 2. 表结构

表名和字段名是本设计的实现契约。时间字段采用 UTC Unix 秒；未知元信息使用 `NULL`，不以 `0` 表示未知。所有写入均使用参数化 SQL；协议专用 JSON 在 Service/Parser 校验后保存，具体键由 04 设计给出。
...
| `url` | `TEXT NOT NULL` | 当前订阅 HTTPS URL |
...
允许新建后暂时没有 Node。不同 Subscription 可以使用相同名称或 URL，各自独立保存筛选条件与 Node。保存时校验 URL 格式与 HTTPS 协议；发起请求时校验证书。
...
| `config_json` | `TEXT NOT NULL` | 已校验的协议专用参数，不重复保存上列公共字段 |

`subscription_id` 外键使用 `ON DELETE RESTRICT`；删除 Subscription 前，Service 按第 5 章完成其 Node 的级联处理。订阅 Node 只能由所属 Subscription 的 Sync 写入；自建 Node 才能由 Node 管理操作直接修改。导入分享 URI 时保存解析后的字段，不保存原始 URI。
...
这个约束保留大小写、前后空白和原始 Unicode 字符，不对名称做自动修剪或归一化；不同 Subscription 及自建 Node 可以重名。Sync 的 Python 比较也使用原始 `name`，与数据库索引保持一致。
...
| `config_json` | `TEXT NOT NULL` | 已校验的认证及协议专用参数 |

`UNIQUE(listen_address, listen_port)` 防止相同地址和端口重复；通配地址与具体地址之间的重叠由 Service 按第 3.3 节校验。Inbound 即使暂未被 Route 引用，也参与监听冲突检查。

### 2.4 `outbounds` 与 `outbound_nodes`
...
`outbound_nodes` 对 Outbound 使用 `ON DELETE CASCADE`，对 Node 使用 `ON DELETE RESTRICT`。前者让删除 Outbound 时一并移除成员行；后者要求 Service 先计算 Node 删除的业务结果。`PRIMARY KEY(outbound_id, node_id)` 防止 Pool 内重复 Node，`UNIQUE(outbound_id, priority)` 防止 priority 重复。
...
这使 Outbound、Pool 成员和 Default Node 可以在同一个事务中建立或修改，并在提交时确认 Default Node 仍属于该 Pool。Service 在提交前另行确认每个保留的 Outbound 至少有两个不同 Node。MANUAL 与 AUTO 转换只更新 `type`，Pool、priority、Default Node 和引用它的 Route 保持原关联。
...
数据库约束要求 `target_kind='direct'` 时 `target_outbound_id IS NULL`；`target_kind='outbound'` 时必须有有效 Outbound ID。目标 Outbound 的 MANUAL/AUTO 类型由 `outbounds.type` 读取，Route 不保存一份可能过时的类型。DIRECT 在 Service 和 API 的系统标识为 `direct`；不向 `outbounds` 插入哨兵记录。

例如，指向 DIRECT 的 Route 保存 `('direct', NULL)`；指向 ID 为 7 的 MANUAL/AUTO 保存 `('outbound', 7)`。当该 Outbound 在 MANUAL 与 AUTO 间转换时，Route 行无需改变。

Route 对 Inbound 和 Outbound 的外键使用 `ON DELETE RESTRICT`。Service 删除 Inbound 或 Outbound 时，先把将被删除的 Route 纳入预览及同一事务，再删除其引用对象。多个 Route 可以指向同一个 Outbound。

### 2.6 数据库基础约束

数据库初始化采用 WAL；每个 SQLite 连接启用 `PRAGMA foreign_keys=ON` 和 `busy_timeout=5000` 毫秒。数据库约束负责外键、枚举、端口范围、唯一成员、唯一 priority、Default Node 成员关系和 Route 目标形态；Service 负责跨行规则、协议字段校验、监听地址重叠及最终 Pool 大小。写事务由 Service 确定，DB 模块接受同一连接执行具体 SQL，操作结束时关闭连接。
...
Pool 成员移除只删除 `outbound_nodes` 的关联行，Node 仍是全局实体。删除全局 Node 才触发它在所有 Pool 中的成员移除及第 5 章的级联判断。

### 3.2 Node Pool、priority 与 Default Node

创建 MANUAL/AUTO 时在一个事务内写入 Outbound、至少两个不同 Node 的 Pool 和 Default Node。未显式选择 Default Node 时，选当前 priority 最小的成员。正常编辑 Pool 后仍须至少两个成员；如果原 Default Node 被移出，选剩余 priority 最小的成员。删除全局 Node 时的不足两成员处理见第 5 章。
...
保存 Inbound 时将监听地址解析为规范化 IPv4 或 IPv6 字面量，并与其他 Inbound、ProxyHub Web 监听位置比较。端口不同则不冲突；端口相同按以下规则判定：
...
## 4. Subscription 请求、解析与差异

### 4.1 输入与处理顺序

Subscription Sync 使用当前保存的 HTTPS URL，校验证书并发送 Clash 兼容 User-Agent。旧分支已经支持普通 URI 行列表、Base64 包装的 URI 列表和含 `proxies` 的 Clash YAML；本版先沿用这三种订阅容器格式作为设计候选，具体格式识别需以脱敏的实际订阅响应验证。Node 只接受 V1.0 的五种协议；单条分享 URI 的参数组合由 04 设计定义。
...
这个计算同样用于 Subscription 删除和 Sync 的 Node 删除部分。普通 Pool 编辑则按第 3.2 节校验，不能通过编辑把成员数减到两个以下。

### 5.3 Preview、Confirm 与事务
...
**仅 DIRECT 的 Route。** Inbound 3 的 Route 保存 `target_kind='direct'`、`target_outbound_id=NULL`。数据库中没有 DIRECT Outbound 行，这条 Route 仍是有效的启动配置来源。
